...
class wf_dhl_ecommerce_shipping_admin{
    ...
    */
    public function elex_dhl_eCommerce_delete_label(){
        ...
        $order_shipment_ids = get_post_meta($this->order_id, 'wf_woo_dhl_ecommerceshipmentId', true);
        foreach($order_shipment_ids as $order_shipment_id){
            delete_post_meta($order_id, 'wf_woo_dhl_ecommerceshippingLabel_' . $order_shipment_id);
            delete_post_meta($order_id, 'wf_woo_dhl_ecommerceshipping_commercialInvoice_' . $order_shipment_id);
        }
        delete_post_meta($order_id, 'wf_woo_dhl_ecommerceservice_code');
        delete_post_meta($order_id, 'wf_woo_dhl_ecommerceshipmentId');
    ...
    public function wf_dhl_emetabox_content(){
        ...
        if (!empty($shipmentIds)) {
            ...
            if(!empty($stored_shipment_dhl_packages)){
                ...
                $available_shipping_services = get_post_meta($post->ID, "available_shipment_services_ecommerce_dhl_elex", true);
            }
