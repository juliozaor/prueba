...
class wf_dhl_woocommerce_shipping_method extends WC_Shipping_Method {
    ...
    public function generate_wf_dhl_tab_box_html() {
       ...
       public function wf_dhl_shipping_page_tabs($current = 'general')
       {
            ...
            if(ELEX_DHL_EXPRESS_AUTO_LABEL_GENERATE_ADDON_WOOCOMMERCE_EXTENSION){
                $tabs['auto-generate-add-on'] =  __("Auto Label Generate Add-on",'wf-shipping-dhl');
            }
            if(ELEX_DHL_INDIA_ADDON_WOOCOMMERCE_EXTENSION){
    ...
    */
    ...
    private function get_dhl_requests($dhl_packages, $package, $package_total_amt='') {
        ...
        */
        ...
        $switch_account_number_action_input = array('account_number' => $this->settings['account_number'], 'source_country_code' => $source_country_code, 'payment_country_code' => $this->settings['dutypayment_country']);
...
XML;
    ...
    */
        ...
        if ($response && !empty($response_services)) {
            foreach ($response_services as $response_service) {
                ...
                if (!in_array($rate_code,$processed_ratecode)) {
                    ...
                    if($rate_cost > 0) {
                        $this->prepare_rate($rate_code, $rate_id, $rate_name, $rate_cost, $delivery_date_time, $rate_cost_weight, $extra_charges['insurance_charge'], $extra_charges['remote_area_surcharge'], $shipping_rates_source_currency, $extra_charges['other_charges']);
                    }
                }
            }
    ...
    private function prepare_rate($rate_code, $rate_id, $rate_name, $rate_cost, $delivery_time, $rate_cost_weight = '0', $dhl_insurance, $dhl_remote_area_surcharge, $shipping_rates_source_currency, $other_charges) {
