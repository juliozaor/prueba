...
class WF_Boxpack {
    ...
    public function pack() {
        try {
            ...
            $current_order = get_option('current_order_data');
...
class WF_Boxpack_Box {
    ...
    public function pack($items, $mode = null) {
        ...
        while (sizeof($items) > 0) {
            ...
            $packed_value += isset($item->final_price) ? $item->final_price : $item->get_value();
        ...
        if ($mode == 'new_algorithm') {
            ...
            // Removed checking of unpacked weight and volume
            if (($packed_weight > 0) && ($packed_volume > 0)) {
