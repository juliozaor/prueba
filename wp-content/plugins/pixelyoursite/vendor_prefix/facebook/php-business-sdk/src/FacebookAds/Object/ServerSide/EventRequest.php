...
class EventRequest implements \ArrayAccess
{
    ...
    public function execute()
    {
        ...
        if ($this->http_client != null) {
            ...
            $http_client = \PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\HttpServiceClientConfig::getInstance()->getClient();
        }
        if ($http_client != null) {
    ...
    private function defaultExecute()
    {
        ...
        $ads_pixel = new \PYS_PRO_GLOBAL\FacebookAds\Object\AdsPixel($this->container['pixel_id']);
        $response = $ads_pixel->createEvent($fields, $normalized_param);
        $event_response = new \PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\EventResponse($response->exportAllData());
        return $event_response;
    }
    private function httpClientExecute($http_client)
    {
        $base_url = 'https://graph.facebook.com/v' . \PYS_PRO_GLOBAL\FacebookAds\ApiConfig::APIVersion;
        $url = $base_url . '/' . $this->container['pixel_id'] . '/events';
        $headers = array('User-Agent' => 'fbbizsdk-php-v' . \PYS_PRO_GLOBAL\FacebookAds\ApiConfig::SDKVersion, 'Accept-Encoding' => '*');
        $curl_options = array(\CURLOPT_CONNECTTIMEOUT => 10, \CURLOPT_TIMEOUT => 60, \CURLOPT_RETURNTRANSFER => \true, \CURLOPT_HEADER => \true, \CURLOPT_CAINFO => \PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\Util::getCaBundlePath());
        $params = $this->normalize();
        if (\PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\HttpServiceClientConfig::getInstance()->getAccessToken() == null) {
            $params['access_token'] = \PYS_PRO_GLOBAL\FacebookAds\Api::instance()->getSession()->getAccessToken();
        } else {
            $params['access_token'] = \PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\HttpServiceClientConfig::getInstance()->getAccessToken();
        }
        $appsecret = null;
        if (\PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\HttpServiceClientConfig::getInstance()->getAppsecret() == null) {
            $appsecret = \PYS_PRO_GLOBAL\FacebookAds\Api::instance()->getSession()->getAppSecret();
        } else {
            $appsecret = \PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\HttpServiceClientConfig::getInstance()->getAppsecret();
        }
        if ($appsecret != null) {
            $params['appsecret_proof'] = \PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\Util::getAppsecretProof($params['access_token'], $appsecret);
        }
        return $http_client->executeRequest($url, \PYS_PRO_GLOBAL\FacebookAds\Object\ServerSide\HttpMethod::POST, $curl_options, $headers, $params);
    }
    /**
     * Normalize
     * @return array
